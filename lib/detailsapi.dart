...
Detailsapi detailsapiFromJson(String str) => Detailsapi.fromJson(json.decode(str));
String detailsapiToJson(Detailsapi data) => json.encode(data.toJson());
class Detailsapi {
  Detailsapi({
      List<Meals>? meals,}){
    _meals = meals;
  ...
  Detailsapi.fromJson(dynamic json) {
    if (json['meals'] != null) {
      _meals = [];
  ...
  List<Meals>? _meals;
Detailsapi copyWith({  List<Meals>? meals,
}) => Detailsapi(  meals: meals ?? _meals,
);
...
Meals copyWith({  String? idMeal,
  ...
  strDrinkAlternate: strDrinkAlternate ?? _strDrinkAlternate,
